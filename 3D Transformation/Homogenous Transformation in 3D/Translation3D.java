...
class Translation3D extends Frame {
    ...
    public Translation3D() {
        try (Scanner sc = new Scanner(System.in)) {
            ...
            this.setTitle("3D Translation (Homogeneous Coordinates)");
            this.setLayout(null);
        ...
        translatedVertices = new int[8][4]; // 4 because we are using homogeneous coordinates
        for (int i = 0; i < 8; i++) {
            translatedVertices[i] = applyTransformation(originalVertices[i], translationMatrix);
        }
    ...
    // Apply the translation matrix to a vertex using matrix multiplication
