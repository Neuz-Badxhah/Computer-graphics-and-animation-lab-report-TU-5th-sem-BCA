...
public class Rect_HomogeneousScaling_2D extends Canvas {

    int x, y, width, height;
    double sx, sy;

    public Rect_HomogeneousScaling_2D() {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            ...
            height = sc.nextInt();
            System.out.println("Enter the scaling factor on the x-axis (sx):");
            sx = sc.nextDouble();
            System.out.println("Enter the scaling factor on the y-axis (sy):");
            sy = sc.nextDouble();
        }
    }
    ...
    public void paint(Graphics g) {
        ...
        // Original rectangle (without scaling)
        ...
        int originalLabelX = midX + x + width / 2;
        int originalLabelY = midY - y + height / 2;
        g2d.drawString("Original", originalLabelX, originalLabelY);

        // Apply homogeneous scaling matrix
        int[][] scaledVertices = new int[4][3];
        for (int i = 0; i < 4; i++) {
            scaledVertices[i] = multiplyMatrixWithScaling(originalVertices[i], sx, sy);
        }

        // Draw the scaled rectangle
        g2d.setColor(Color.RED);
        drawRectangle(g2d, scaledVertices, midX, midY);
        // Position the "Scaled" label at the center of the scaled rectangle
        int scaledLabelX = (scaledVertices[0][0] + scaledVertices[2][0]) / 2 + midX;
        int scaledLabelY = (scaledVertices[0][1] + scaledVertices[2][1]) / 2;
        g2d.drawString("Scaled", scaledLabelX, midY - scaledLabelY);
        ...
        g2d.drawString("Scaling factor sx: " + sx, 50, 150);
        g2d.drawString("Scaling factor sy: " + sy, 50, 170);
    }

    // Method to multiply a point with the scaling matrix
    private int[] multiplyMatrixWithScaling(int[] vertex, double sx, double sy) {
        int[] result = new int[3];
        result[0] = (int) (vertex[0] * sx);
        result[1] = (int) (vertex[1] * sy);
        result[2] = 1;
        return result;
    }
    ...
    public static void main(String[] args) {
        ...
        JFrame frame = new JFrame("Rectangle Scaling using Homogeneous Transformation");
        Rect_HomogeneousScaling_2D canvas = new Rect_HomogeneousScaling_2D();
