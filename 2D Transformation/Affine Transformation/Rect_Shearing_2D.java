...
public class Rect_Shearing_2D extends Canvas {

    int x, y, width, height;
    double shearX, shearY;

    public Rect_Shearing_2D() {
    ...
    public void paint(Graphics g) {
        ...
        // Apply the shearing (shearX, shearY) to the rectangle
        AffineTransform shear = new AffineTransform();
        shear.shear(shearX, shearY); // Apply the shear transformation
        g2d.setTransform(shear); // Set the transformation
        g2d.setColor(Color.RED);
        g2d.draw(rect); // Draw the sheared rectangle
        g2d.drawString("Sheared", rect.x + rect.width + 10, rect.y + rect.height / 2);
        ...
        g2d.drawString("Shear factor X: " + shearX, 50, 150);
        g2d.drawString("Shear factor Y: " + shearY, 50, 170);
    }

    public static void main(String[] args) {
        // Create a JFrame window to display the canvas
        JFrame frame = new JFrame("Rectangle Shearing in 2D");
        Rect_Shearing_2D canvas = new Rect_Shearing_2D();
